...
class PasswordAuth {
  ...
  Future<String?> usuario(
      {required String email, required String password}) async {
    var userCredential = await FirebaseAuth.instance.currentUser!.displayName;
  ...
  Future<bool> signUp({required UserModel userModel}) async {
    try {
      ...
      userCredential.user!.updateDisplayName(userModel.nickName);
      FirestoreDatabase firestoreDatabase = Get.find();
